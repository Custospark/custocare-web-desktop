...
const BillingActionCenter: React.FC<BillingActionCenterProps> = ({ theme }) => {
  ...
  const handleWorkOnAnotherPatient = async () => {
    try {
      ...
      navigate(BILLING_ROUTES.PATIENT_QUEUE);
    } catch (error) {
  ...
  if (isNavigating) {
    return (
      <LoadingSkeleton
        variant="default"
        theme={theme}
        message="Getting the billing queue ready for your next patient..."
      />
    );
  }
    ...
    return (
      ...
      >
        <div className="max-w-2xl text-center">
          ...
          <h2 className="mb-3 text-2xl font-bold">No active billing encounter</h2>
          <p className={`mb-6 text-base ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
            Start from Billing Queue or Express Intake to load a visit into Billing Encounter.
          </p>

          <div className="flex flex-col justify-center gap-3 sm:flex-row">
            <button
              type="button"
              onClick={() => navigate(BILLING_ROUTES.PATIENT_QUEUE)}
            ...
            <button
              type="button"
              onClick={() => navigate(BILLING_ROUTES.WALKIN_PATIENT)}
...
interface BillingPatientInfoCardProps {
  theme: 'light' | 'dark';
...
const BillingPatientInfoCard: React.FC<BillingPatientInfoCardProps> = ({
  theme,
  patient,
  visitInfo,
  calculateWaitTime,
    ...
    >
      ...
      <div className={`space-y-2 border-t p-4 ${isDark ? 'border-gray-800 bg-gray-800/30' : 'border-gray-200 bg-gray-50'}`}>
        <button
          ...
          title={isNavigating ? 'Navigating to billing queue...' : 'Switch to work on another patient'}
        >
          {isNavigating ? (
            <>
