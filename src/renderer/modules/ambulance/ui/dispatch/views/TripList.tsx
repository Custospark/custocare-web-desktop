...
import { Clock, RefreshCw, Search, Activity, PlusCircle } from 'lucide-react';
...
const TripList = ({ theme }: TripListProps) => {
  ...
  const { data, refetch, isFetching } = useTrips({ search: search || undefined, per_page: 50 });
  const trips = data?.data ?? [];

  const handleSelect = (trip: AmbulanceTrip) => {
        ...
        {trips.length === 0 ? (
          ...
          <table className="w-full">
            <thead><tr className={`border-b text-left text-xs font-semibold uppercase tracking-wider ${isDark ? 'border-gray-800 bg-gray-800/50 text-gray-400' : 'border-gray-200 bg-gray-50 text-gray-500'}`}>
              <th className="px-4 py-3">Type</th><th className="px-4 py-3">Progress</th><th className="px-4 py-3">Route</th><th className="px-4 py-3">Priority</th><th className="px-4 py-3">Date</th><th className="px-4 py-3 text-right">Actions</th>
            </tr></thead>
