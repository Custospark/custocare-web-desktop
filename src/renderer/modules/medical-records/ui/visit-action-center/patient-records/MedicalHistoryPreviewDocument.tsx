...
const InfoRow: React.FC<{
  ...
  <div className="flex items-center justify-between border-b border-gray-200 py-2 last:border-0 print:py-1.5">
    <span className="flex items-center gap-2 text-xs font-medium text-gray-700 print:text-[10px]">
      <span className="print:hidden">{icon}</span>
      {label}
    </span>
    <span className="text-right text-xs font-semibold text-gray-900 print:text-[10px]">{value}</span>
  </div>
);
...
function FacilityCell({ facility }: { facility: FacilitySnapshot | null | undefined }) {
  ...
  return (
    <div className="text-xs">
      ...
      {facility.code ? (
        <span className="ml-1 text-gray-600">({facility.code})</span>
      ) : null}
      {facility.address?.formatted ? (
        <p className="mt-0.5 text-[10px] leading-snug text-gray-600">{facility.address.formatted}</p>
      ) : null}
    </div>
  );
}
...
export const MedicalHistoryPreviewDocument: React.FC<MedicalHistoryPreviewDocumentProps> = ({ history }) => {
  ...
  if (isLoading && shouldFetch && !!activeFacilityId) {
    return (
      <div className="mx-auto max-w-4xl rounded-2xl border border-gray-200 bg-white p-6 shadow-sm print:max-w-none print:rounded-none print:border-0 print:p-0">
        <p className="text-center text-sm text-gray-700">Loading document…</p>
      </div>
    );
  }

  return (
    <div className="mx-auto max-w-4xl rounded-2xl border border-gray-200 bg-white p-6 shadow-sm print:max-w-none print:rounded-none print:border-0 print:p-0 print:shadow-none">
      <div className="mb-6 border-b-2 border-blue-700 pb-5 text-center print:mb-4 print:pb-4">
        <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-100 print:hidden">
        ...
        <div className="mt-2 flex flex-wrap items-center justify-center gap-2 print:mt-1">
          <span className="rounded-full bg-blue-100 px-2.5 py-1 text-[11px] font-semibold text-blue-800 print:bg-transparent print:p-0 print:text-gray-700">
            Continuity of Care
          </span>
          <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-[11px] font-semibold text-emerald-800 print:bg-transparent print:p-0 print:text-gray-700">
            Aggregated Medical History
          </span>
        </div>

        <div className="mt-3 space-y-1 text-xs text-gray-700 print:mt-2">
        ...
        <div className="mt-3 print:mt-2">
          <span className="inline-block rounded-full bg-gray-100 px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-gray-600 print:bg-transparent print:p-0 print:text-gray-500">
            Facility Number:{' '}
            <span className="text-gray-800 print:text-gray-900">{viewerFacility.code}</span>
          </span>
        </div>

        <div className="mt-4 rounded-xl bg-linear-to-r from-blue-100 to-emerald-100 px-4 py-3 print:mt-3 print:rounded-none print:border-y print:border-gray-300 print:bg-transparent print:py-2">
          <p className="text-lg font-black tracking-wide text-blue-800 print:text-base print:text-gray-900">
      ...
      <div className="my-6 space-y-2 rounded-lg bg-gray-100 p-4 print:my-4 print:border print:border-gray-300 print:bg-transparent print:p-3">
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        <h3 className="mb-3 flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-gray-800">
          <Calendar className="h-4 w-4 text-blue-700" />
          Encounters (all facilities)
        </h3>
        {history.visits.length === 0 ? (
          <p className="text-sm text-gray-600">No visits on file.</p>
        ) : (
          <table className="w-full border-collapse border border-gray-300 text-sm">
            <thead className="bg-gray-100">
              <tr>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">When</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Facility</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Type / status</th>
              </tr>
            </thead>
            <tbody>
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.allergies.length === 0 ? (
          <p className="text-sm text-gray-600">No allergy records.</p>
        ) : (
          <table className="w-full border-collapse border border-gray-300 text-sm">
            <thead className="bg-gray-100">
              <tr>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Allergen</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Severity</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Facility</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Recorded</th>
              </tr>
            </thead>
            <tbody>
              {history.allergies.map((a) => (
                <tr key={a.id} className="align-top">
                  <td className="border border-gray-300 px-2 py-2 text-xs font-medium text-gray-900">
                    {a.allergen}
                    {a.reaction ? <div className="mt-1 font-normal text-gray-700">{a.reaction}</div> : null}
                  </td>
                  <td className="border border-gray-300 px-2 py-2 text-xs text-gray-700">{a.severity ?? '—'}</td>
                  <td className="border border-gray-300 px-2 py-2">
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.prescriptions.length === 0 ? (
          <p className="text-sm text-gray-600">No prescriptions documented.</p>
        ) : (
          <div className="space-y-4">
            {history.prescriptions.map((rx) => (
              <div key={rx.id} className="rounded-xl border border-gray-300 p-3 print:break-inside-avoid">
                <div className="flex flex-wrap items-start justify-between gap-2 border-b border-gray-200 pb-2">
                  <div>
                    <p className="text-sm font-semibold text-gray-900">{rx.prescription_number}</p>
                ...
                {rx.diagnosis ? <p className="mt-2 text-xs text-gray-700">Dx context: {rx.diagnosis}</p> : null}
                {rx.items.length === 0 ? (
                  <p className="mt-2 text-xs text-gray-600">No line items.</p>
                ) : (
                  <table className="mt-3 w-full border-collapse border border-gray-300 text-xs">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="border border-gray-300 px-2 py-1 text-left text-gray-800 font-medium">Medication</th>
                        <th className="border border-gray-300 px-2 py-1 text-left text-gray-800 font-medium">Sig</th>
                        <th className="border border-gray-300 px-2 py-1 text-left text-gray-800 font-medium">Route</th>
                      </tr>
                    </thead>
                    <tbody>
                      {rx.items.map((it) => (
                        <tr key={it.id}>
                          ...
                          <td className="border border-gray-300 px-2 py-1 text-gray-700">{formatText(it.route) || '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.clinical_notes.length === 0 ? (
          <p className="text-sm text-gray-600">No clinical notes.</p>
        ) : (
          <div className="space-y-3">
            {history.clinical_notes.map((n) => (
              <div key={n.id} className="rounded-xl border border-gray-300 p-3 print:break-inside-avoid">
                <div className="flex flex-wrap justify-between gap-2 border-b border-gray-200 pb-2">
                  <div>
                    <p className="text-sm font-semibold text-gray-900 capitalize">{n.note_type ?? 'Note'} · {n.note_status ?? '—'}</p>
                    <p className="text-xs text-gray-700">
                      {formatDateTime(n.occurred_at ?? n.noted_at ?? n.created_at)}
                    </p>
                    <p className="text-xs text-gray-700">{renderClinician(n.clinician?.name)}</p>
                  </div>
                  <FacilityCell facility={n.facility} />
                </div>
                {n.assessment ? <p className="mt-2 text-xs text-gray-800">{n.assessment}</p> : null}
                {n.plan ? <p className="mt-1 text-xs text-gray-700">Plan: {n.plan}</p> : null}
              </div>
            ))}
          </div>
        )}
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.vitals.length === 0 ? (
          <p className="text-sm text-gray-600">No vitals.</p>
        ) : (
          <table className="w-full border-collapse border border-gray-300 text-sm">
            <thead className="bg-gray-100">
              <tr>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Time</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Facility</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Measurements</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Clinician</th>
              </tr>
            </thead>
            <tbody>
              {history.vitals.map((v) => (
                <tr key={v.id} className="align-top">
                  ...
                  <td className="border border-gray-300 px-2 py-2 text-xs text-gray-800">{vitalSummary(v)}</td>
                  <td className="border border-gray-300 px-2 py-2 text-xs text-gray-700">{renderClinician(v.clinician?.name)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.diagnoses.length === 0 ? (
          <p className="text-sm text-gray-600">No diagnoses recorded.</p>
        ) : (
          <table className="w-full border-collapse border border-gray-300 text-sm">
            <thead className="bg-gray-100">
              <tr>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Diagnosis</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Code</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Facility</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Recorded</th>
              </tr>
            </thead>
              ...
              {history.diagnoses.map((d) => (
                <tr key={d.id} className="align-top">
                  <td className="border border-gray-300 px-2 py-2 text-xs font-medium text-gray-900">
                    ...
                    <div className="font-normal text-gray-700">{d.clinical_status ?? ''}</div>
                    <div className="font-normal text-gray-700">{renderClinician(d.clinician?.name)}</div>
                   </td>
                  <td className="border border-gray-300 px-2 py-2 text-xs text-gray-700">{d.diagnosis_code ?? '—'}</td>
                  <td className="border border-gray-300 px-2 py-2">
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.consultations.length === 0 ? (
          <p className="text-sm text-gray-600">No consultations.</p>
        ) : (
          <div className="space-y-3">
            {history.consultations.map((c) => (
              <div key={c.id} className="rounded-xl border border-gray-300 p-3 print:break-inside-avoid">
                <div className="flex flex-wrap justify-between gap-2 border-b border-gray-200 pb-2">
                  <div>
                    <p className="text-sm font-semibold text-gray-900">{c.clinical_question ?? 'Consultation'}</p>
                    <p className="text-xs text-gray-700">
                ...
                {c.findings ? <p className="mt-2 text-xs text-gray-800">{c.findings}</p> : null}
                {c.recommendations ? <p className="mt-1 text-xs text-gray-700">{c.recommendations}</p> : null}
              </div>
            ))}
          </div>
        )}
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.lab_requests.length === 0 ? (
          <p className="text-sm text-gray-600">No laboratory requests.</p>
        ) : (
          <div className="space-y-4">
            {history.lab_requests.map((lab) => (
              <div key={lab.id} className="rounded-xl border border-gray-300 p-3 print:break-inside-avoid">
                <div className="flex flex-wrap justify-between gap-2 border-b border-gray-200 pb-2">
                  <div>
                    <p className="text-sm font-semibold text-gray-900">Request {lab.request_uuid.slice(0, 8)}…</p>
                    <p className="text-xs text-gray-700">
                      {formatDateTime(lab.occurred_at ?? lab.requested_at)} · {formatText(lab.status) || '—'} ·{' '}
                      {formatText(lab.priority) || '—'}
                    </p>
                    <p className="text-xs text-gray-700">{renderClinician(lab.clinician?.name)}</p>
                  </div>
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.lab_results.length === 0 ? (
          <p className="text-sm text-gray-600">No laboratory results.</p>
        ) : (
          <table className="w-full border-collapse border border-gray-300 text-sm">
            <thead className="bg-gray-100">
              <tr>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Test</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Result</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Flag</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Facility</th>
                <th className="border border-gray-300 px-2 py-2 text-left text-xs font-semibold text-gray-800">Clinician / time</th>
              </tr>
            </thead>
            <tbody>
              {history.lab_results.map((res) => (
                <tr key={res.id} className="align-top">
                  <td className="border border-gray-300 px-2 py-2 text-xs font-medium text-gray-900">
                    ...
                    {res.field_name ? <div className="font-normal text-gray-700">{res.field_name}</div> : null}
                   </td>
                  <td className="border border-gray-300 px-2 py-2 text-xs text-gray-800">
                    {res.value ?? '—'} {res.unit ?? ''}
                  ...
                  <td className="border border-gray-300 px-2 py-2 text-xs text-gray-700">{formatText(res.flag) || '—'}</td>
                  <td className="border border-gray-300 px-2 py-2">
                  ...
                  <td className="border border-gray-300 px-2 py-2 text-xs text-gray-700">
                    ...
                    {res.verified_by?.name ? (
                      <div className="text-gray-600">Verified: {renderClinician(res.verified_by?.name)}</div>
                    ) : null}
                   </td>
                </tr>
              ))}
      ...
      <div className="mt-10 border-t border-gray-300 pt-4 text-center text-[10px] text-gray-500 print:mt-8">
