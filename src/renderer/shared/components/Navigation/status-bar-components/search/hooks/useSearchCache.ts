// search/hooks/useSearchCache.ts
import { useState, useCallback, useEffect } from 'react';

// ─── Constants ────────────────────────────────────────────────────────────────
const CACHE_KEY = 'hms_global_search_v1';
...
interface CacheData {
  ...
  moduleHits: Record<string, ModuleHit>;
}
...
export function useSearchCache() {
  ...
  // ── Write: wipe everything (recent + module hits) ─────────────────────────
  const clearAllHistory = useCallback(() => {
    mutate(() => EMPTY_CACHE);
  }, []);

  // ── Write: increment visit count for a module ─────────────────────────────
  const recordModuleVisit = useCallback((moduleId: string) => {
    mutate((prev) => {
      const existing = prev.moduleHits[moduleId];
      return {
        ...prev,
        moduleHits: {
          ...prev.moduleHits,
  ...
  // ── Read: top N module ids by visit count ────────────────────────────────
  const getTopModuleIds = useCallback(
    (count: number): string[] =>
      Object.values(globalCache.moduleHits)
