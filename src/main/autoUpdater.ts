...
// We will explicitly install on quit (more reliable than relying on autoInstallOnAppQuit)
...
export function initAutoUpdater(mainWindow: BrowserWindow): void {
  ...
  autoUpdater.on('update-downloaded', (info) => {
    // IMPORTANT: downloaded = ready to install, NOT installed
    ...
    showNativeNotification(
      'Update Ready',
      `Version ${info.version} will be installed when you exit the app.`
    );
  });
...
/**
 * Called from main.ts (before-quit).
 * Returns true if it triggered quitAndInstall and took over the quit flow.
 */
export function installUpdateOnQuitIfReady(): boolean {
  if (isDev) return false;
  if (!updateReadyToInstall) return false;
  if (installingNow) return true; // already in progress / quit already handled

  installingNow = true;

  try {
    log.info('[Auto-Updater] Running quitAndInstall() to apply update...');
    // isSilent=true, isForceRunAfter=false (we want user to restart manually later)
