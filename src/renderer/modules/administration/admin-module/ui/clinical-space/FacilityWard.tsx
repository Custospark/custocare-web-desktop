    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
        ...
        <div className={cn(
          ...
          <div className="absolute bottom-3 right-3">
            <div className={cn(
              ...
              <span>+{normalizedWards.length > 0 ? Math.floor(normalizedWards.length * 0.15) : 0}%</span>
          ...
          <div className="absolute bottom-3 right-3 w-16">
            <div className="h-1.5 bg-gray-700/30 rounded-full overflow-hidden">
              <div 
                ...
                style={{ 
                  width: `${normalizedWards.length > 0 
                    ? (normalizedWards.filter(w => w.status === WardStatus.ACTIVE).length / normalizedWards.length) * 100 
                    : 0}%` 
                }}
              />
            </div>
        ...
        <div className={cn(
          ...
          {/* Sub-info showing declared capacity */}
          <div className="absolute bottom-3 right-3">
            <div className={cn(
              'text-xs',
              isDark ? 'text-gray-500' : 'text-gray-400'
            )}>
              Declared: {normalizedWards.reduce((sum, ward) => sum + (ward.capacity_declared || 0), 0)}
            </div>
          </div>
        </div>
        ...
        <div className={cn(
          ...
          {/* Show filter badges when no search but filters active */}
            ...
            <div className="absolute bottom-3 right-3 flex gap-1">
              {typeFilter !== 'all' && (
                <span className={cn(
                  ...
                  {getWardTypeLabel(typeFilter as WardType).split(' ')[0]}
                </span>
              )}
              {statusFilter !== 'all' && (
                <span className={cn(
