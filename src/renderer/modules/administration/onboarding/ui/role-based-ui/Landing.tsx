...
import { Heart, Stethoscope, CheckCircle2, Building, UserCheck, Sparkles } from 'lucide-react';
import { useAppSelector } from '../../../../../app/store/hooks/useApp';
import { cn } from '../../../../../shared/types/cn';
import { containerVariants, itemVariants, cardHoverVariants } from '../../../../../shared/components/animations/motionVariants';
      ...
      <div className="w-full max-w-7xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          >
            ...
            >
              {[
                { icon: Stethoscope, title: 'For Care Teams', description: 'Stop toggling between systems. One login gives doctors, nurses, and pharmacists unified access to clinical, pharmacy, lab, and billing.', gradient: 'from-blue-500 to-blue-600', color: 'blue' },
                { icon: UserCheck, title: 'For Patients', description: 'No more repeating your history. One centralized record follows you across every department and facility — every visit.', gradient: 'from-emerald-500 to-emerald-600', color: 'emerald' },
                { icon: Building, title: 'For Health Facilities', description: 'Single facility or 50 locations. Digital queues cut wait times. Integrated billing stops revenue leakage. One platform, full visibility.', gradient: 'from-purple-500 to-purple-600', color: 'purple' },
              ].map((benefit, index) => (
                <motion.div
                  key={index}
                  variants={itemVariants}
                  ...
                  animate="rest"
                  className={cn(
                    "group relative p-5 rounded-2xl border transition-all duration-400 cursor-pointer overflow-hidden min-h-[220px]",
                ...
                >
                  {theme === 'dark' && (
                    <div className={cn("absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl",
                  ...
                  <motion.div
                    ...
                    className={cn("inline-flex items-center justify-center w-11 h-11 rounded-xl mb-4 bg-gradient-to-br shadow relative z-10", benefit.gradient,
                      theme === 'dark' ? "shadow-md group-hover:shadow-lg" : "shadow-sm group-hover:shadow",
                  ...
                  <h3 id={`benefit-${index}-title`} className={cn("font-bold mb-2 text-base sm:text-lg relative z-10 transition-colors duration-300",
                    theme === 'dark' ? cn(benefit.color === 'blue' && "group-hover:text-blue-300", benefit.color === 'emerald' && "group-hover:text-emerald-300", benefit.color === 'purple' && "group-hover:text-purple-300", "text-white") : cn(benefit.color === 'blue' && "group-hover:text-blue-800", benefit.color === 'emerald' && "group-hover:text-emerald-800", benefit.color === 'purple' && "group-hover:text-purple-800", "text-slate-900"))}>
                    {benefit.title}
                  </h3>
                  <p className={cn("text-sm leading-relaxed relative z-10 transition-colors duration-300",
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex -space-x-2.5" role="img" aria-label="Platform trust indicators">
                  {[1, 2, 3, 4].map((i) => (
                    <motion.div
                      key={i}
                ...
                <div className="text-left">
                  ...
                  <div className="flex items-center gap-1.5">
                    {[...Array(5)].map((_, i) => (
                      <motion.div key={i} initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 1.1 + i * 0.04, type: "spring" }}>
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" aria-hidden="true" />
                      </motion.div>
                    ))}
                    <span className={cn("text-xs font-semibold ml-1", theme === "dark" ? "text-slate-400" : "text-slate-600")}>
        ...
        {isLoading && (
          ...
          >
            <motion.div
              ...
              className={cn("p-10 rounded-3xl flex flex-col items-center gap-6 border-2 shadow-2xl max-w-sm mx-4",
            ...
            >
              <div className="relative">
                <motion.div
                  ...
                  className={cn("w-20 h-20 border-[4px] rounded-full",
                    theme === 'dark' ? "border-blue-500/30 border-t-blue-500 shadow-lg shadow-blue-500/20" : "border-blue-500/30 border-t-blue-600 shadow-lg shadow-blue-500/30"
                  )}
                  aria-hidden="true"
                />
                <motion.div
              ...
              <div className="text-center space-y-2">
                <motion.p className={cn("font-bold text-xl tracking-tight", theme === 'dark' ? "text-white" : "text-slate-900")}
                  animate={{ opacity: [1, 0.6, 1] }} transition={{ duration: 1.8, repeat: Infinity }}>
                  {isLoading === 'login' ? 'Accessing Your Dashboard' : 'Preparing Your Healthcare Journey'}
                </motion.p>
