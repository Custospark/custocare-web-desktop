...
import { DischargePreviewDocument } from './DischargePreviewDocument';
import LoadingSkeleton from '../../../../../../shared/components/Loading/LoadingSkeletons';
import type { DischargeData, DischargePreviewAction } from './dischargeForm.types';

interface DischargePreviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  dischargeData: DischargeData | null;
  patientName?: string;
  theme?: 'light' | 'dark';
  initialAction?: DischargePreviewAction;
  isLoading?: boolean;
}

export const DischargePreviewModal: React.FC<DischargePreviewModalProps> = ({
  isOpen,
  onClose,
  dischargeData,
  ...
  const documentTitle = useMemo(() => {
    ...
    const safePatientName = name
      .toLowerCase()
      .replace(/[^\w\s-]/g, '')
      .trim()
      .replace(/\s+/g, '-');
    const todayDate = new Date().toISOString().split('T')[0];
    return `${safePatientName}_discharge_${todayDate}`;
  }, [dischargeData, patientName]);
  ...
  }, [handleDownload, handlePrint, initialAction, isOpen]);
      ...
      <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/70 p-0 sm:items-center sm:p-4">
        <div className="no-print flex h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:h-[92vh] sm:rounded-2xl">
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 print:hidden">
            <div>
              <h3 className="text-base font-semibold text-slate-900">Discharge Summary Preview</h3>
          ...
          <div className="flex-1 overflow-y-auto bg-slate-100 p-4 sm:p-6">
            <LoadingSkeleton variant="default" message="Loading discharge summary..." />
          </div>
        </div>
      </div>
    );
      ...
      >
        <div className="no-print flex h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:h-[92vh] sm:rounded-2xl">
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 print:hidden">
            <div>
              <h3 className="text-base font-semibold text-slate-900">Discharge Summary Preview</h3>
              <p className="mt-1 text-sm text-slate-600">No discharge summary found</p>
            </div>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto bg-slate-100 p-4 sm:p-6">
            <div className="flex flex-col items-center justify-center rounded-lg bg-white p-12 text-center">
              ...
              <h3 className="mb-2 text-lg font-semibold text-slate-900">No Discharge Summary Found</h3>
              <p className="text-sm text-slate-600">
                No discharge summary found for {patientName} yet.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 print:hidden">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Review the discharge summary before printing or saving as PDF.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={() => handlePrint?.()}
              className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-teal-600 px-4 py-2 text-sm font-medium text-white transition-all hover:bg-teal-700"
            >
