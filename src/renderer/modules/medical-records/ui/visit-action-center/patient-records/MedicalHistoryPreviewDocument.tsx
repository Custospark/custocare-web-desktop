...
function FacilityCell({ facility }: { facility: FacilitySnapshot | null | undefined }) {
  ...
  return (
    <div className="text-xs">
      ...
      {facility.code ? (
        <span className="ml-1 text-slate-500">({facility.code})</span>
      ) : null}
      {facility.address?.formatted ? (
        <p className="mt-0.5 text-[10px] leading-snug text-slate-500">{facility.address.formatted}</p>
      ) : null}
    </div>
  );
}
  ...
  if (isLoading && shouldFetch && !!activeFacilityId) {
    return (
      <div className="mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm print:max-w-none print:rounded-none print:border-0 print:p-0">
        <p className="text-center text-sm text-slate-600">Loading document…</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm print:max-w-none print:rounded-none print:border-0 print:p-0 print:shadow-none">
      <div className="mb-6 border-b-2 border-blue-600 pb-5 text-center print:mb-4 print:pb-4">
        ...
        <div className="mt-4 rounded-xl bg-linear-to-r from-blue-50 to-emerald-50 px-4 py-3 print:mt-3 print:rounded-none print:border-y print:border-gray-200 print:bg-transparent print:py-2">
          <p className="text-lg font-black tracking-wide text-blue-700 print:text-base print:text-slate-900">
            MEDICAL HISTORY SUMMARY
          </p>
          <p className="mt-1 text-xs font-medium text-slate-600 print:text-[10px]">
            Documented records across facilities for longitudinal care coordination.
          </p>
        </div>
      </div>

      <div className="my-6 space-y-2 rounded-lg bg-gray-50 p-4 print:my-4 print:border print:border-gray-200 print:bg-transparent print:p-3">
        ...
        {history.visits.length === 0 ? (
          ...
          <table className="w-full border-collapse border border-slate-200 text-sm">
            <thead className="bg-slate-100">
              <tr>
                <th className="border border-slate-200 px-2 py-2 text-left text-xs font-semibold">When</th>
                <th className="border border-slate-200 px-2 py-2 text-left text-xs font-semibold">Facility</th>
                <th className="border border-slate-200 px-2 py-2 text-left text-xs font-semibold">Type / status</th>
              </tr>
            </thead>
            <tbody>
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.allergies.length === 0 ? (
          <p className="text-sm text-slate-500">No allergy records.</p>
        ) : (
          <table className="w-full border-collapse border border-slate-200 text-sm">
            <thead className="bg-slate-100">
              <tr>
                <th className="border border-slate-200 px-2 py-2 text-left text-xs font-semibold">Allergen</th>
                <th className="border border-slate-200 px-2 py-2 text-left text-xs font-semibold">Severity</th>
                <th className="border border-slate-200 px-2 py-2 text-left text-xs font-semibold">Facility</th>
                <th className="border border-slate-200 px-2 py-2 text-left text-xs font-semibold">Recorded</th>
              </tr>
            </thead>
            <tbody>
              {history.allergies.map((a) => (
                <tr key={a.id} className="align-top">
                  <td className="border border-slate-200 px-2 py-2 text-xs font-medium">
                    {a.allergen}
                    {a.reaction ? <div className="mt-1 font-normal text-slate-600">{a.reaction}</div> : null}
                  </td>
                  <td className="border border-slate-200 px-2 py-2 text-xs">{a.severity ?? '—'}</td>
                  <td className="border border-slate-200 px-2 py-2">
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.prescriptions.length === 0 ? (
          <p className="text-sm text-slate-500">No prescriptions documented.</p>
        ) : (
          <div className="space-y-4">
            {history.prescriptions.map((rx) => (
              <div key={rx.id} className="rounded-xl border border-slate-200 p-3 print:break-inside-avoid">
                ...
                {rx.diagnosis ? <p className="mt-2 text-xs text-slate-600">Dx context: {rx.diagnosis}</p> : null}
                {rx.items.length === 0 ? (
                  ...
                  <table className="mt-3 w-full border-collapse border border-slate-200 text-xs">
                    <thead className="bg-slate-50">
                      <tr>
                        <th className="border border-slate-200 px-2 py-1 text-left">Medication</th>
                        <th className="border border-slate-200 px-2 py-1 text-left">Sig</th>
                        <th className="border border-slate-200 px-2 py-1 text-left">Route</th>
                      </tr>
                    </thead>
                    <tbody>
                      {rx.items.map((it) => (
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.clinical_notes.length === 0 ? (
          <p className="text-sm text-slate-500">No clinical notes.</p>
        ) : (
            ...
            {history.clinical_notes.map((n) => (
              <div key={n.id} className="rounded-xl border border-slate-200 p-3 print:break-inside-avoid">
                <div className="flex flex-wrap justify-between gap-2 border-b border-slate-100 pb-2">
                  <div>
                    <p className="text-sm font-semibold capitalize">{n.note_type ?? 'Note'} · {n.note_status ?? '—'}</p>
                    <p className="text-xs text-slate-500">
                ...
                {n.assessment ? <p className="mt-2 text-xs text-slate-700">{n.assessment}</p> : null}
                {n.plan ? <p className="mt-1 text-xs text-slate-600">Plan: {n.plan}</p> : null}
              </div>
            ))}
          </div>
        )}
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.vitals.length === 0 ? (
          <p className="text-sm text-slate-500">No vitals.</p>
        ) : (
          <table className="w-full border-collapse border border-slate-200 text-sm">
            <thead className="bg-slate-100">
              <tr>
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.diagnoses.length === 0 ? (
          ...
          <table className="w-full border-collapse border border-slate-200 text-sm">
            <thead className="bg-slate-100">
              <tr>
                <th className="border border-slate-200 px-2 py-2 text-left text-xs font-semibold">Diagnosis</th>
                <th className="border border-slate-200 px-2 py-2 text-left text-xs font-semibold">Code</th>
                <th className="border border-slate-200 px-2 py-2 text-left text-xs font-semibold">Facility</th>
                <th className="border border-slate-200 px-2 py-2 text-left text-xs font-semibold">Recorded</th>
              </tr>
            </thead>
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.consultations.length === 0 ? (
          <p className="text-sm text-slate-500">No consultations.</p>
        ) : (
          <div className="space-y-3">
            {history.consultations.map((c) => (
              <div key={c.id} className="rounded-xl border border-slate-200 p-3 print:break-inside-avoid">
                ...
                {c.findings ? <p className="mt-2 text-xs text-slate-700">{c.findings}</p> : null}
                {c.recommendations ? <p className="mt-1 text-xs text-slate-600">{c.recommendations}</p> : null}
              </div>
            ))}
          </div>
        )}
      ...
      <section className="mt-8 print:mt-6 print:break-inside-avoid">
        ...
        {history.lab_requests.length === 0 ? (
          <p className="text-sm text-slate-500">No laboratory requests.</p>
        ) : (
          <div className="space-y-4">
      ...
      <div className="mt-10 border-t border-slate-200 pt-4 text-center text-[10px] text-slate-400 print:mt-8">
