...
interface VehicleCreateProps { theme: 'light' | 'dark'; }

const INITIAL = {
  vehicle_identifier: '', vehicle_type: 'bls', equipment_level: '',
  status: 'available', current_mileage: 0, capacity: 1,
...
const VehicleCreate = ({ theme }: VehicleCreateProps) => {
  ...
  const set = useCallback((field: string) => (e: FormEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
  ...
  const inputClass = `w-full rounded-lg border px-3 py-2 text-sm outline-none transition-all ${
    isDark ? 'border-gray-700 bg-gray-800 text-gray-100 focus:border-blue-500' : 'border-gray-200 bg-white text-gray-900 focus:border-blue-500'
  }`;
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <input className={`${inputClass} ${errors.vehicle_identifier ? 'border-red-500' : ''}`} placeholder="e.g. AMB-001" value={form.vehicle_identifier} onChange={set('vehicle_identifier')} />
                {errors.vehicle_identifier && <p className="mt-1 text-xs text-red-500">{errors.vehicle_identifier}</p>}
              </div>
              ...
              <div>
                ...
                <input className={inputClass} placeholder="e.g. basic, advanced, critical" value={form.equipment_level} onChange={set('equipment_level')} />
              </div>

              <div>
                <label className={labelClass}>Capacity</label>
                <input type="number" min={1} max={255} className={inputClass} value={form.capacity} onChange={set('capacity')} />
              </div>
              ...
              <div className="col-span-2">
                ...
                <input className={inputClass} placeholder="e.g. stretcher, defibrillator, oxygen" value={form.features} onChange={set('features')} />
              </div>
            </div>

            <div className="flex justify-end gap-3 pt-4">
              <button type="button" onClick={() => navigate(AMBULANCE_ROUTES.VEHICLES_ALL)}
                className={`cursor-pointer rounded-lg border px-4 py-2 text-sm font-medium ${isDark ? 'border-gray-700 text-gray-300 hover:bg-gray-800' : 'border-gray-200 text-gray-700 hover:bg-gray-50'}`}>
                Cancel
              </button>
              <button type="submit" disabled={createMutation.isPending}
                ...
                <Save className="h-4 w-4" /> {createMutation.isPending ? 'Saving...' : 'Register Ambulance'}
              </button>
            </div>
          </form>
        </div>
