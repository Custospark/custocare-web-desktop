...
export interface UserPermissionsMetricItem {
  title: string;
  value: string;
  subtitle: string;
...
interface UserPermissionsMetricsGridProps {
  isDark: boolean;
  metrics: UserPermissionsMetricItem[];
}
...
const UserPermissionsMetricsGrid: React.FC<UserPermissionsMetricsGridProps> = ({
  isDark,
  metrics,
}) => {
  return (
      ...
      {metrics.map((metric, index) => {
        ...
        return (
          ...
          >
            <div
              className={cn(
                'group relative h-full cursor-pointer overflow-hidden rounded-xl p-5 transition-all duration-300',
                'border-2',
            ...
            >
              <div className="absolute inset-0 rounded-xl bg-gradient-to-t from-black/5 to-transparent dark:from-white/5" />

              <div
                className={cn(
                  'absolute -right-10 -top-10 h-32 w-32 rounded-full blur-2xl transition-all duration-500',
                  accentStyles.bgBlur,
              ...
              <div
                className={cn(
                  'absolute -bottom-10 -left-10 h-32 w-32 rounded-full blur-2xl transition-all duration-500',
                  accentStyles.bgBlur,
              ...
              <div className="relative z-10 mb-4 flex items-start justify-between">
                ...
                >
                  <metric.icon className={cn('h-5 w-5', accentStyles.iconColor)} />
                </div>
              ...
              <div className="relative z-10 mt-3">
                <p
                  className={cn(
                    'break-words whitespace-normal text-3xl font-bold tracking-tight sm:text-4xl',
                ...
                <p
                  className={cn(
                    'mt-2 break-words whitespace-normal text-sm font-semibold',
                ...
                <p
                  className={cn(
                    'mt-1.5 break-words whitespace-normal text-xs',
              ...
              <div
                className={cn(
                  ...
                  `bg-gradient-to-r ${accentStyles.gradient
                    .replace('to-gray-900', '')
                    .replace('to-white', '')}`,
              ...
              <div className="absolute bottom-4 right-4 opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:scale-110">
                <div className={cn('h-2 w-2 rounded-full', accentStyles.iconColor)} />
              </div>
            </div>
          </motion.div>
        );
...
export default UserPermissionsMetricsGrid;
