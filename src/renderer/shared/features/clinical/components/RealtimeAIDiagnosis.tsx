...
const generateDifferentials = (
  ...
  const hasBodyPain = symptoms.some(s => 
    s.name.toLowerCase().includes('pain') || s.name.toLowerCase().includes('ache')
  );
  const hasCough = symptoms.some(s => s.name.toLowerCase().includes('cough'));
  const hasAbdominalPain = symptoms.some(s => 
    s.name.toLowerCase().includes('abdomen') || s.name.toLowerCase().includes('stomach')
  );

  // Generate differentials based on symptoms
  if (hasFever || hasHeadache) {
    differentials.push({
      ...
      evidence: [
        ...
        ...(hasBodyPain ? ['Body aches consistent'] : [])
      ],
      suggestedTests: ['Malaria RDT', 'Blood smear', 'Complete Blood Count'],
      contextFactors: [
        {
          type: 'geographic',
          factor: 'Malaria-endemic zone',
          weight: 0.3,
          description: 'High malaria prevalence region'
        }
      ],
      updatedAt: now
    });
  ...
  if (hasFever || hasAbdominalPain) {
    differentials.push({
      ...
      suggestedTests: ['Widal test', 'Blood culture', 'Complete Blood Count'],
      contextFactors: [
        {
          type: 'geographic',
          factor: 'Endemic region',
          weight: 0.25,
          description: 'Poor sanitation infrastructure area'
        }
      ],
      updatedAt: now
    });
  ...
  if (hasFever && hasBodyPain) {
    differentials.push({
      ...
      suggestedTests: ['Dengue NS1 antigen', 'Complete Blood Count', 'Platelet count'],
      contextFactors: [
        {
          type: 'seasonal',
          factor: 'Rainy season',
          weight: 0.2,
          description: 'Peak mosquito breeding'
        }
      ],
      updatedAt: now
    });
...
export const RealtimeAIDiagnosis: React.FC<RealtimeAIDiagnosisProps> = ({
  ...
  const prevInputsRef = useRef({ chiefComplaint, symptoms, vitals });
  ...
  const updateDiagnosis = useCallback(() => {
    ...
    updateTimeoutRef.current = setTimeout(() => {
      ...
      newDifferentials.forEach(newDiff => {
        const oldDiff = differentials.find(d => d.condition === newDiff.condition);
        if (oldDiff && oldDiff.probability !== newDiff.probability) {
          changes.push({
            ...
            reason: newDiff.probability > oldDiff.probability 
              ? 'New evidence supports diagnosis' 
      ...
      if (changes.length > 0) {
        const newUpdate: DiagnosisUpdate = {
  ...
  }, [chiefComplaint, symptoms, vitals, differentials, isProcessing]);

  // Debounced effect for updates
  useEffect(() => {
    const currentInputs = { chiefComplaint, symptoms, vitals };
    const prevInputs = prevInputsRef.current;
    
    const hasInputChanged = 
      currentInputs.chiefComplaint !== prevInputs.chiefComplaint ||
      JSON.stringify(currentInputs.symptoms) !== JSON.stringify(prevInputs.symptoms) ||
      currentInputs.vitals !== prevInputs.vitals;
    
    const hasInput = chiefComplaint || symptoms.length > 0 || vitals;
    
    if (hasInput && hasInputChanged) {
      // updateDiagnosis();
  ...
  }, [chiefComplaint, symptoms, vitals, updateDiagnosis]);
  ...
  const hasData = differentials.length > 0;
  ...
  return (
    <div className="space-y-4 sticky top-4">
      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={cn('p-2 rounded-lg', theme === 'dark' ? 'bg-cyan-500/20' : 'bg-blue-100')}>
              <Brain className={cn('w-6 h-6', theme === 'dark' ? 'text-cyan-400' : 'text-blue-600')} />
            </div>
            <div>
              <h3 className={cn('font-bold text-lg', theme === 'dark' ? 'text-white' : 'text-gray-900')}>
                AI Differential Diagnosis
              </h3>
              <p className={cn('text-xs', theme === 'dark' ? 'text-cyan-300/80' : 'text-blue-600/80')}>
          ...
          {isProcessing && (
            <div className="flex items-center gap-2">
              <Zap className={cn('w-4 h-4 animate-pulse', theme === 'dark' ? 'text-cyan-400' : 'text-blue-600')} />
              <span className={cn('text-xs font-medium', theme === 'dark' ? 'text-cyan-300' : 'text-blue-600')}>
                Processing...
              </span>
            </div>
          )}
        ...
        {lastUpdateTime && (
          <div className={cn('mt-3 text-xs flex items-center gap-2', theme === 'dark' ? 'text-gray-400' : 'text-gray-600')}>
      ...
      {/* Context Factors */}
      {hasData && differentials[0]?.contextFactors?.length > 0 && (
        <div className={cn('rounded-xl border p-4', theme === 'dark' ? 'bg-gray-900/50 border-gray-800' : 'bg-white border-gray-200')}>
          <h4 className={cn('text-sm font-semibold mb-3', theme === 'dark' ? 'text-gray-200' : 'text-gray-800')}>
            Context Factors
          </h4>
          <div className="grid grid-cols-1 gap-2">
            {differentials[0].contextFactors.map((factor, idx) => (
              <div key={idx} className={cn('p-2 rounded-lg text-xs', theme === 'dark' ? 'bg-gray-800/50' : 'bg-gray-50')}>
      ...
      {hasData ? (
        <div className="space-y-3">
          {differentials.map((diff, index) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <span className={cn('text-2xl font-bold', theme === 'dark' ? 'text-cyan-400' : 'text-blue-600')}>
                      #{index + 1}
                    </span>
                    <h4 className={cn('font-bold text-base', theme === 'dark' ? 'text-white' : 'text-gray-900')}>
                      {diff.condition}
                    </h4>
                  </div>
                  
                  <span className={cn('text-xs px-2 py-1 rounded-full font-medium', getConfidenceColor(diff.confidence))}>
              ...
              {/* Probability Bar */}
              <div className="mb-3">
                <div className={cn('h-2 rounded-full overflow-hidden', theme === 'dark' ? 'bg-gray-800' : 'bg-gray-200')}>
                  <div
                    className={cn('h-full transition-all duration-500', getProbabilityColor(diff.probability))}
              ...
              {/* Evidence */}
              <div className="mb-3">
                <div className={cn('text-xs font-semibold mb-1', theme === 'dark' ? 'text-gray-400' : 'text-gray-600')}>
                  Supporting Evidence:
                </div>
                <ul className="space-y-1">
                  {diff.evidence.map((item, idx) => (
                    <li key={idx} className={cn('text-xs flex items-start gap-2', theme === 'dark' ? 'text-gray-300' : 'text-gray-700')}>
                      <CheckCircle className={cn('w-3 h-3 mt-0.5 flex-shrink-0', theme === 'dark' ? 'text-green-400' : 'text-green-600')} />
                      {item}
                    </li>
                  ))}
                </ul>
              ...
              {/* Suggested Tests */}
              <div>
                <div className={cn('text-xs font-semibold mb-1 flex items-center gap-1', theme === 'dark' ? 'text-gray-400' : 'text-gray-600')}>
        ...
        <div className={cn('rounded-xl border p-8 text-center', theme === 'dark' ? 'bg-gray-900/50 border-gray-800' : 'bg-white border-gray-200')}>
          <Brain className={cn('w-12 h-12 mx-auto mb-3', theme === 'dark' ? 'text-gray-600' : 'text-gray-400')} />
      ...
      {updates.length > 0 && (
        <div className={cn('rounded-xl border p-4', theme === 'dark' ? 'bg-gray-900/50 border-gray-800' : 'bg-white border-gray-200')}>
          <div className="flex items-center gap-2 mb-3">
            <TrendingUp className={cn('w-4 h-4', theme === 'dark' ? 'text-cyan-400' : 'text-blue-600')} />
          ...
          <div className="space-y-2">
            {updates.slice(0, 3).map((update, idx) => (
              <div key={idx} className={cn('text-xs p-2 rounded-lg', theme === 'dark' ? 'bg-gray-800/50' : 'bg-gray-50')}>
      ...
      <div className={cn(
        ...
        <div className="flex items-start gap-2">
          <AlertCircle className={cn('w-4 h-4 flex-shrink-0 mt-0.5', theme === 'dark' ? 'text-yellow-400' : 'text-yellow-600')} />
          <p className={cn(theme === 'dark' ? 'text-yellow-300/90' : 'text-yellow-700')}>
