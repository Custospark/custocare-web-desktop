...
import { LabRequestPreviewDocument } from './LabRequestPreviewDocument';
import type { LabRequest } from '../../../../api/lab/LabTypes';
import LoadingSkeleton from '../../../../../../shared/components/Loading/LoadingSkeletons';

interface LabRequestPreviewModalProps {
  open: boolean;
  onClose: () => void;
...
export const LabRequestPreviewModal: React.FC<LabRequestPreviewModalProps> = ({
  open,
  onClose,
  request,
  ...
  // Generate proper filename: patient_name_lab_request_YYYY-MM-DD
  const documentTitle = useMemo(() => {
    ...
    // Build filename: patientName_lab-request_YYYY-MM-DD
    const fileName = `${safePatientName}_lab-request_${todayDate}`;
    
    return fileName || 'lab-request';
  }, [request]);
      ...
      <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/70 p-0 sm:items-center sm:p-4">
        <div className="no-print flex h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:h-[92vh] sm:rounded-2xl">
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 print:hidden">
            <div>
              <h3 className="text-base font-semibold text-slate-900">Laboratory Request Preview</h3>
          ...
          <div className="flex-1 overflow-y-auto bg-slate-100 p-4 sm:p-6">
            <LoadingSkeleton variant="default" message="Loading laboratory request report..." />
          </div>
        </div>
      </div>
    );
      ...
      >
        <div className="no-print flex h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:h-[92vh] sm:rounded-2xl">
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 print:hidden">
            <div>
              <h3 className="text-base font-semibold text-slate-900">Laboratory Request Preview</h3>
              <p className="mt-1 text-sm text-slate-600">No laboratory request report found</p>
            </div>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto bg-slate-100 p-4 sm:p-6">
            <div className="flex flex-col items-center justify-center rounded-lg bg-white p-12 text-center">
              ...
              <h3 className="mb-2 text-lg font-semibold text-slate-900">No Laboratory Request Found</h3>
              <p className="text-sm text-slate-600">No laboratory request report found for {patientName} yet.</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="no-print flex h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:h-[92vh] sm:rounded-2xl">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 print:hidden">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Review the laboratory request before printing or saving as PDF.
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-100 p-4 sm:p-6">
          <div ref={contentRef}>
            <LabRequestPreviewDocument request={request} />
          </div>
        </div>
...
export default LabRequestPreviewModal;