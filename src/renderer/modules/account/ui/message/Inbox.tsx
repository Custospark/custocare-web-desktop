/**
 * ============================================================================
 * INBOX COMPONENT — REAL API INTEGRATION
 * ============================================================================
 */
...
import {
  ...
  Paperclip, Calendar, Filter, Search, RefreshCw,
} from 'lucide-react';
...
/* Internal normalised display shape — keeps render code clean */
interface DisplayMessage {
  id: number;
  subject: string;
  senderName: string;
  senderLabel: string;     // display name / email-hash fallback
...
const mapToDisplay = (item: MessageStateWithMessage): DisplayMessage => {
  ...
  return {
    ...
    senderLabel: msg.sender?.email_hash
...
/* Map UI filter → API filter / folder params */
...
export const Inbox: React.FC<InboxProps> = ({ theme }) => {
  ...
  const starredCount = messages.filter(m => m.starred).length; // from current page
  ...
  const handleReply = useCallback(() => {
    ...
    navigate(ACCOUNT_ROUTES.MESSAGES_INBOX); // navigate to compose with replyTo
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
        className={cn('relative overflow-hidden rounded-xl border-2 mb-4 transition-all duration-300 group',
          isDark ? 'bg-gradient-to-br from-gray-800 to-gray-900 border-blue-500/30 hover:border-blue-500/50'
            : 'bg-gradient-to-br from-white to-blue-50/50 border-blue-200 hover:border-blue-400')}>
        <div className={cn('absolute top-0 right-0 w-64 h-64 rounded-full blur-3xl transition-opacity opacity-0',
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className={cn('p-3 rounded-xl transition-all duration-300',
                isDark ? 'bg-blue-500/20 group-hover:bg-blue-500/30 group-hover:scale-110'
              ...
              <div>
                <h2 className="text-xl font-bold flex items-center gap-2">
                  ...
                  <span className={cn('text-sm font-medium px-2.5 py-1 rounded-full',
                    isDark ? 'bg-blue-500/20 text-blue-300 border border-blue-500/30'
            ...
            <div className="flex items-center gap-2">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                ...
                className={cn('inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border-2 transition-all disabled:opacity-50 cursor-pointer',
                  isDark ? 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-700 hover:border-gray-600'
              ...
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                ...
                className={cn('inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border-2 transition-all md:hidden cursor-pointer',
                  isDark ? 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-700 hover:border-gray-600'
                    : 'bg-gray-50 border-gray-200 text-gray-700 hover:bg-gray-100 hover:border-gray-300')}>
      ...
      <div className="hidden md:block mb-4">
        <div className={cn('relative overflow-hidden rounded-xl border-2 transition-all duration-300',
          isDark ? 'bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700/50 hover:border-gray-600'
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 relative">
                ...
                <div className="relative z-10 m-[2px] rounded-[6px] overflow-hidden">
                  <Search className={cn('absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors duration-200',
                  ...
                  <input type="text" placeholder="Search messages..." value={searchTerm}
                    ...
                    className={cn('w-full pl-10 pr-10 py-2 text-sm border-transparent focus:outline-none focus:ring-0 transition-colors',
                      isDark ? 'bg-gray-900 text-white placeholder-gray-500' : 'bg-white text-gray-900 placeholder-gray-400')} />
                  {searchTerm && (
                    <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}
                      onClick={handleClearSearch}
                      className={cn('absolute right-2.5 top-1/2 -translate-y-1/2 p-1 rounded-full transition-colors cursor-pointer',
                        isDark ? 'text-gray-400 hover:text-gray-200 hover:bg-gray-700'
              ...
              <div className="flex items-center gap-1">
                {([
                  ...
                  <button key={key} onClick={() => setFilter(key)}
                    className={cn('px-3 py-2 rounded-lg text-sm font-medium transition-all border-2 cursor-pointer',
                      filter === key
              ...
              <select value={sort} onChange={e => setSort(e.target.value as LocalSort)}
                className={cn('px-3 py-2 rounded-lg text-sm border-2 cursor-pointer',
      ...
      <AnimatePresence>
        {showFilters && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
            ...
            <div className={cn('rounded-xl border-2 p-3',
              ...
              <div className="space-y-3">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search messages..." value={searchTerm}
                    onChange={e => setSearchTerm(e.target.value)}
                    className={cn('w-full pl-10 pr-10 py-2 rounded-lg text-sm border-2 focus:outline-none focus:ring-2 focus:ring-blue-500',
                      isDark ? 'bg-gray-900 border-gray-700 text-white placeholder-gray-500'
                ...
                <div className="flex flex-wrap gap-2">
                  {([
                    ...
                    <button key={key} onClick={() => setFilter(key)}
                      className={cn('px-3 py-1.5 rounded-lg text-xs font-medium border-2',
                        filter === key
                          ? isDark ? 'bg-blue-600 border-blue-500 text-white' : 'bg-blue-600 border-blue-400 text-white'
                          : isDark ? 'bg-gray-700 border-gray-600 text-gray-300'
                ...
                <select value={sort} onChange={e => setSort(e.target.value as LocalSort)}
                  className={cn('w-full px-3 py-2 rounded-lg text-sm border-2',
      ...
      <div className="flex-1 min-h-0 flex gap-4">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
          className={cn('w-full md:w-96 flex-shrink-0 overflow-hidden rounded-xl border-2 transition-all',
            !showMobileList && 'hidden md:block',
            isDark ? 'bg-gradient-to-br from-gray-800 to-gray-900 border-gray-700/50'
              : 'bg-gradient-to-br from-white to-gray-50/50 border-gray-200')}>
          <div className="h-full flex flex-col">
            {/* list header */}
            <div className={cn('p-3 border-b-2 flex items-center justify-between',
              isDark ? 'border-gray-700' : 'border-gray-200')}>
              <span className={cn('text-sm font-medium', isDark ? 'text-gray-300' : 'text-gray-700')}>
                {messages.length} messages
              </span>
              {debouncedSearch && <span className={cn('text-xs', isDark ? 'text-gray-500' : 'text-gray-500')}>Filtered</span>}
            </div>
                  ...
                  {messages.length === 0 ? (
                    ...
                    messages.map((msg, index) => (
                      <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        className={cn('p-4 cursor-pointer transition-all relative group',
                          ...
                          <div className="flex items-center gap-2 min-w-0 flex-1">
                            ...
                            <div className="min-w-0 flex-1">
                              <p className={cn('text-sm truncate', !msg.read && 'font-semibold',
                                isDark ? 'text-white' : 'text-gray-900')}>{msg.senderName}</p>
                              <p className={cn('text-xs truncate', isDark ? 'text-gray-400' : 'text-gray-600')}>{msg.senderLabel}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-1 flex-shrink-0">
                            {msg.priority === 'high' && <AlertCircle className={cn('w-3 h-3', isDark ? 'text-red-400' : 'text-red-500')} />}
                            <span className={cn('text-xs whitespace-nowrap', isDark ? 'text-gray-500' : 'text-gray-500')}>{msg.timestamp}</span>
                          </div>
                        </div>
                        {/* Subject */}
                        <div className="flex items-start gap-2 mb-1">
                          {!msg.read && <span className="w-2 h-2 rounded-full bg-blue-500 mt-1.5 flex-shrink-0" />}
                          <p className={cn('text-sm truncate flex-1', !msg.read && 'font-semibold',
                            isDark ? 'text-white' : 'text-gray-900')}>{msg.subject}</p>
                        </div>
                        {/* Preview */}
                        <p className={cn('text-xs truncate mb-2', isDark ? 'text-gray-400' : 'text-gray-600')}>{msg.preview}</p>
                        {/* Labels */}
                        <div className="flex flex-wrap items-center gap-1">
                          {msg.labels.map(label => (
                            <span key={label} className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs',
                        ...
                        <button onClick={e => handleToggleStar(msg.id, e)}
                          className={cn('absolute top-4 right-4 p-1 rounded-full transition-all opacity-0 group-hover:opacity-100',
                            msg.starred && 'opacity-100', isDark ? 'hover:bg-gray-600' : 'hover:bg-gray-200')}>
                          <Star className={cn('w-4 h-4', msg.starred ? 'fill-yellow-400 text-yellow-400' : isDark ? 'text-gray-500' : 'text-gray-400')} />
                        </button>
                      </motion.div>
                    ))
                  )}
        ...
        {/* Detail panel */}
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
          className={cn('flex-1 min-w-0 overflow-hidden rounded-xl border-2 transition-all',
          ...
          {selectedMessage ? (
            <div className="h-full flex flex-col">
              {/* Detail header */}
              <div className={cn('p-4 border-b-2 flex items-center justify-between',
                isDark ? 'border-gray-700' : 'border-gray-200')}>
                <div className="flex items-center gap-2">
                  ...
                  {selectedMessage.priority === 'high' && (
                    <span className={cn('px-2 py-0.5 rounded-full text-xs font-medium',
                      isDark ? 'bg-red-900/20 text-red-300 border border-red-500/30'
                ...
                <div className="flex items-center gap-2">
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                    ...
                    <Star className={cn('w-5 h-5', selectedMessage.starred
                      ? 'fill-yellow-400 text-yellow-400' : isDark ? 'text-gray-500' : 'text-gray-400')} />
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  ...
                  <div className="relative group">
                    ...
                    <div className={cn('absolute right-0 top-full mt-1 w-48 rounded-lg border-2 shadow-lg overflow-hidden z-10 hidden group-hover:block',
                      isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200')}>
              ...
              <div className="flex-1 overflow-y-auto p-6">
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className={cn('text-lg font-semibold', isDark ? 'text-white' : 'text-gray-900')}>{selectedMessage.senderName}</h4>
                      <p className={cn('text-sm', isDark ? 'text-gray-400' : 'text-gray-600')}>{selectedMessage.senderLabel}</p>
                      <p className={cn('text-xs mt-1', isDark ? 'text-gray-500' : 'text-gray-500')}>To: {selectedMessage.recipientNames}</p>
                    </div>
                  </div>
                ...
                {selectedMessage.attachments.length > 0 && (
                  <div className="mt-6">
                    <h5 className={cn('text-sm font-medium mb-3 flex items-center gap-2',
                      isDark ? 'text-gray-300' : 'text-gray-700')}>
                      <Paperclip className="w-4 h-4" /> Attachments ({selectedMessage.attachments.length})
                    </h5>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {selectedMessage.attachments.map((att, idx) => (
                        <div key={idx} className={cn('p-3 rounded-lg border-2 flex items-center gap-3',
              ...
              <div className={cn('p-4 border-t-2', isDark ? 'border-gray-700' : 'border-gray-200')}>
                <div className="flex items-center gap-3">
                  <input type="text" placeholder="Type your reply..." value={replyText}
                    onChange={e => setReplyText(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && !e.shiftKey && handleReply()}
                    className={cn('flex-1 px-4 py-2 rounded-lg border-2 focus:outline-none focus:ring-2 focus:ring-blue-500',
                      isDark ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400'
                        : 'bg-white border-gray-200 text-gray-900 placeholder-gray-500')} />
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                    onClick={handleReply}
                    className={cn('px-4 py-2 rounded-lg font-medium cursor-pointer',
            ...
            <div className="h-full flex items-center justify-center p-8">
              <div className="text-center">
                ...
                <p className={cn('text-sm', isDark ? 'text-gray-400' : 'text-gray-600')}>Select a message from the list to view it</p>
              </div>
            </div>
          )}
        </motion.div>
