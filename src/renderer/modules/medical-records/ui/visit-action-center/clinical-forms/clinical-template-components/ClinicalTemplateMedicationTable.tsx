...
import type { TemplateMedicationItem } from '../../../../api/clinical-templates/ClinicalTemplateTypes';
import {
    DosageForm,
  DosageUnit,
  DurationUnit,
...
interface ClinicalTemplateMedicationTableProps {
  isDark: boolean;
  colors: {
    bg: {
      subtle: string;
...
export const ClinicalTemplateMedicationTable: React.FC<
  ClinicalTemplateMedicationTableProps
> = ({
  isDark,
  colors,
  medications,
  ...
  emptyDescription = 'Add medications to complete this clinical template.',
}) => {
  const showActions = !!onEditMedication || !!onDeleteMedication;
  ...
  if (medications.length === 0) {
    return (
      ...
      >
        ...
        <p className={cn('text-sm font-medium', colors.text.primary)}>{emptyTitle}</p>
        <p className={cn('mt-1 text-sm', colors.text.secondary)}>{emptyDescription}</p>
      </div>
    );
  }
        ...
        <table className="w-full min-w-[980px] border-collapse">
          <thead>
            <tr className={cn('border-b', colors.border.primary)}>
              ...
              {showActions && <th className={cn(tableHeaderClass, 'text-center')}>Actions</th>}
            </tr>
          </thead>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-start gap-2">
                      <span className="mt-0.5 text-lg" aria-hidden="true">
                        {getDosageFormIcon(med.dosage_form as DosageForm)}
                      </span>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn('text-sm font-medium', colors.text.primary)}>
                        {formatDosage(med.dosage_quantity, med.dosage_unit as DosageUnit)}
                      </span>
                    </div>

                    {med.dosage_form && (
                      <div className={cn('text-sm', colors.text.secondary)}>
                        Form: {med.dosage_form as DosageForm}
                      </div>
                    )}
                  </div>
                </td>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className={cn('text-sm', colors.text.secondary)}>
                      {med.duration_value && med.duration_unit
                        ? formatDuration(med.duration_value, med.duration_unit as DurationUnit)
                        : '—'}
                    </div>
                  </div>
                </td>
                ...
                <td className="px-4 py-4">
                  <div className="max-w-sm space-y-2">
                    ...
                    {med.administration_instructions &&
                      med.administration_instructions !== 'None' && (
                        <div className={cn('text-sm', colors.text.secondary)}>
                          Administration: {med.administration_instructions}
                        </div>
                      )}
...
export default ClinicalTemplateMedicationTable;
